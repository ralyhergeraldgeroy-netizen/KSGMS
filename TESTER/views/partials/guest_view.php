                ...
                <?php foreach ($guest_bookings as $order): ?>
                    <div class="bg-black border border-neutral-800 rounded-xl p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 hover:border-yellow-500/50 transition-all">
                        <div class="space-y-1">
                            ...
                            <span class="block text-[10px] font-mono text-neutral-600">Check-out date: <?= !empty($order['check_out']) ? htmlspecialchars(date('M d, Y', strtotime($order['check_out']))) : 'N/A' ?></span>
                        </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div class="bg-neutral-950 border-2 border-neutral-800 rounded-xl p-5">
                ...
                <input
                    ...
                    value="<?= date('Y-m-d') ?>"
                    required
                    class="w-full px-4 py-3 bg-black border border-neutral-700 rounded-lg text-white text-sm focus:border-yellow-400 focus:outline-none cursor-pointer"
                >

                <label for="checkoutDate" class="block text-xs font-black uppercase tracking-widest text-yellow-400 mb-3 mt-5">
