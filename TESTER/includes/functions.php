...
function getGuestBookings($pdo, $username) {
    try {
        $stmt = $pdo->prepare("SELECT id, resort_name, room_number, total_price, food_items, check_in, check_out, remarks FROM bookings WHERE customer_name = ? ORDER BY id DESC");
...
function getAdminData($pdo) {
    ...
    try {
        ...
        $bookings_query = $pdo->query("SELECT id, customer_name, resort_name, room_number, total_price, food_items, check_in, check_out, remarks FROM bookings ORDER BY id DESC");
